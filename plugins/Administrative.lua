...
if (mr_roo[1]:lower() == "panelsudo" or mr_roo[1] == "پنل سودو") and is_admin(msg) and is_JoinChannel(msg) then
	...
	tdbot.getInlineQueryResults(BotSuperZedlink_idapi, msg.to.id, 0, 0, "Sudo:"..msg.to.id, 0, inline_query_cb, nil)
end
--######################################################################--
...
local text = mr_roo[2]
tdbot.createNewChannelChat(text, 1, '@BeroozTeam', (function(b, d) tdbot.addChatMember(d.id, msg.from.id, 0, dl_cb, nil) end), nil)
...
if (mr_roo[1]:lower() == 'panelgp' or mr_roo[1] == 'پنل گروه') and is_admin(msg) and is_JoinChannel(msg) then
	...
	tdbot.getInlineQueryResults(BotSuperZedlink_idapi, msg.to.id, 0, 0, "Menu:"..mr_roo[2], 0, inline_query_cb, nil)
end
--######################################################################--
if (mr_roo[1]:lower() == 'codegift' or mr_roo[1] == 'کدهدیه') and is_sudo(msg) and is_JoinChannel(msg) then
	...
	local codetext = "SuperZedlink-"..a..b..c..d..e..f.."-TeaM"
...
run = BeroozTeam, pre_process = pre_processTools
}
