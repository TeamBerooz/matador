...
if msg.query and msg.query:sub(1,6) == "Menu:-" and msg.query:gsub("Menu:-",""):match('%d+') and is_sudo(msg) then
	...
	keyboard.inline_keyboard = {
		{
            {text = "❤️ "..tostring(redis:get(RedisIndex.."SuperZedlinkLikes")), callback_data="/like:"..chatid},
            {text = "💔 "..tostring(redis:get(RedisIndex.."SuperZedlinkDisLikes")), callback_data="/dislike:"..chatid}
        },
		{
...
if msg.cb then
  ...
  if matches[1] == '/like' then
      if redis:get(RedisIndex.."IsDisLiked:"..msg.from.id) then
        ...
        local dislikes = redis:get(RedisIndex.."SuperZedlinkDisLikes")
        redis:set(RedisIndex.."SuperZedlinkDisLikes",dislikes - 1)
        redis:set(RedisIndex.."IsLiked:"..msg.from.id,true)
        local likes = redis:get(RedisIndex.."SuperZedlinkLikes")
        redis:set(RedisIndex.."SuperZedlinkLikes",likes + 1)
